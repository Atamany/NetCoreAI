        ...
        static async Task<string> AdvancedSentimentalAnalysis(string text)
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                var requestBody = new
                {
                    model = "gpt-3.5-turbo",
                    messages = new[] {
                        new { role = "system", content = "You are an avanced AI that analyzes emotions in text. Your response must be in JSON format. Identify the sentiment scores (0-100%) for the following emotions: Joy, Sadness, Angry, Fear, Surprise, Neutral" },
                ...
                else { Console.WriteLine($"Bir Hata oluştu: {responseJson}"); return "Hata!"; }
            } } }
        }