...
class Program
{
    ...
    {
        Console.WriteLine("RSS Beslemesinden Haberler Alınıyor...");
    ...
    static async Task<List<string>> FetchLatestNews(int count)
    {
        ...
        List<string> articles = items.Select(item => { string title = item.Element("title")?.Value ?? ""; string description = item.Element("description")?.Value ?? ""; return $"{title}. {description}"; }).ToList();
        return articles;
    }
